...
class RAGChain:
    ...
    def _build_qa_prompt(self, context: str, question: str) -> str:
        ...
        # 原有提示词（已注释）
        # return f"""# 角色和要求
        # 你是一个专业的问答助手，下面是知识库检索到的参考内容和用户的问题，请使用知识库参考内容回答用户的问题。必须严格按照知识库参考内容进行回答，不要自己创造。
        #
        # # 约束
        # - 请严格按照知识库检索到的内容进行回复。不需要对检索的内容做过多加工。
        # - 未检索到的内容，请回复抱歉，未检索到知识。
        # - 如果知识库的相关内容包含图片信息，请在输出信息中保留其链接地址，不要做任何修改；如果知识库的相关内容没有包含图片信息，那就输出信息中只包含原文，不要自己编造图片URL。
        #
        # ## 任务
        #
        # 知识库参考内容:
        ...
        # 问题: {question}
        #
        # 请根据以上内容回答用户问题，回答要简洁、准确，并标注参考页码。
        #
        # 特别强调：如果知识库的相关内容原文包含图片占位符（格式：`![描述](http://localhost:8080/...)`），请在回答中完整保留，以便给用户展示。 如果知识库的相关内容没有包含图片占位符，那也不要自己编造。"""
